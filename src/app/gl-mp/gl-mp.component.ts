...
@Component({
  selector: 'app-gl-mp',
  imports: [],
  templateUrl: './gl-mp.component.html',
  styleUrl: './gl-mp.component.scss'
})
export class GlMpComponent {
  map!: maplibregl.Map;
  ...
  initMap(): void {
    ...
    //   forwardGeocode: async (config: MaplibreGeocoderApiConfig): Promise<any> => {
    //     const features: any = [];
    //     try {
    //       debugger
    //       const query = typeof config.query === 'string' ? config.query : ''; // Ensure query is a string for Nominatim
    //       const request =
    //         `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=geojson&polygon_geojson=1&addressdetails=1`;
